    ...
    <div className="lg:hidden">
      ...
      >
        <button
          ...
          className={`absolute inset-0 bg-slate-950/45 backdrop-blur-[2px] transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"}`}
        />

        <aside
          id="mobile-navigation"
          aria-label="モバイルメニュー"
          className={`absolute inset-y-0 left-0 flex w-[min(82vw,304px)] flex-col bg-[#101b36] px-4 py-5 text-white shadow-2xl transition-transform duration-300 ease-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
